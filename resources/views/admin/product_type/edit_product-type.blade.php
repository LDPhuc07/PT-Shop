  ...
  @media(max-width: 767px) {
    ...
    <div class="product-container">
      ...
      <form id="form">
        @method('PUT')
        @csrf
        <input type="hidden" value="{{$dsLoaiSanPham['id']}}" name="id" id="id">
              ...
              $.ajax({
                  ...
                  success:function(data){
                      if(!$.isEmptyObject(data.success)) {
                        location.replace("http://127.0.0.1:8000/admin/loaisanpham");
                        alert(data.success);
                      } 
                      if(!$.isEmptyObject(data.error)) {
                          
                          if(!$.isEmptyObject(data.error.tenloaisanpham)) {
                              printErrorMsg (data.error.tenloaisanpham, 'tenloaisanpham');
